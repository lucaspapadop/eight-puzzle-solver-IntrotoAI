...
/* aSNilson is a heuristic based search that visits nodes of a search space based on the minimum heuristic value
 * of each available, open node in the process of expansion. The heuristics used are a Manhattan distance,
 * the summation of each tile's distance from its correct position in the goal state, and a Nilson value, a value
 * computed by checking that all the tiles have their correct clockwise neighbors (among other things).
 * A priority queue is used to sort and maintain the order of visiting nodes based on this heuristic. See TreeNode and
 * manPriorityQueue classes for an explanation of how priority and heuristics are calculated.
 *
 * A Hashset is constructed as the closed list. Next, an array containing the clockwise path of tiles in the goal state
 * is created, the windMill list. That windMill list is translated and copied into a Hashmap that keeps track of the
 * tiles and their associated neighbor: we call it windMillMap in this class, and neighbors in TreeNode.
 * An array goalIndices is constructed. It keeps track of the indices of each tile in their correct position.
 ...
 * the root node into manPriorityQueue, while also passing in the correct indices of each tile in the goal state and the
 ...
 * permutation of the original board position (the search tree is exhausted without finding the goal position).
 *
    ...
    public Queue manExpansion(int[] currentList, int[] goal) {
        ...
        while (!priorityQueue.isEmpty() && !check) {
            ...
            if (!closedList.contains(Arrays.toString(child1))){
                TreeNode childNode1 = new TreeNode(child1, goalIndices, windMillMap);
                expansionTree.addLeft(currentNode, childNode1);
                priorityQueue.enqueue(childNode1); //at each enqueue, calculate the h(x)
            ...
            if (!closedList.contains(Arrays.toString(child2))) {
                TreeNode childNode2 = new TreeNode(child2, goalIndices, windMillMap);
            ...
            if ((child3 != null) && !closedList.contains(Arrays.toString(child3))) {
                TreeNode childNode3 = new TreeNode(child3, goalIndices, windMillMap);
            ...
            if ((child4 != null) && !closedList.contains(Arrays.toString(child4))) {
                TreeNode childNode4 = new TreeNode(child4,goalIndices, windMillMap);
